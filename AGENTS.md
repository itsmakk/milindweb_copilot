...
When asked to deploy, inspect, query, or manage Render services/databases/logs/metrics, use the `render` MCP server. First render tool call in a session requires the workspace to be set — prompt the user to set it (e.g., "Set my Render workspace to <name>") if not already active.

## GitHub MCP

When asked to read or modify repositories, issues, pull requests, Actions runs, code-scanning/Dependabot alerts, gists, or any other GitHub resource, use the `github` MCP server. Default toolsets (`context`, `repos`, `issues`, `pull_requests`, `users`) are sufficient for most tasks; only narrow with `GITHUB_TOOLSETS` if the user asks or context size becomes a concern.